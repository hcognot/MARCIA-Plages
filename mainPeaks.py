...
def majorPeaks(x_data, y_data):
     ...
     mean, array_peaks = basisLow (array_peaks, y_data)
...
""" limitation of the size of an array ( 4 peaks ) if needed + 
obtention of the mean value of the y_data of the excluded points """
def basisLow (array_pics, y_data):
     # valeur d'attente
     basis=0
     ...
     if len(array_pics)<4:
          ...
          mean = mini - 0.2
          """
          """" retrieve the minimum value of the whole y_data """
          basis = np.min(y_data) - 0.2
          print ('on a moins de 4 pics -> valeur de base est :' , basis)

     if len(array_pics)  == 4:
          mini,k = min_(array_pics, y_data)
          #print('le plus petit des pics a comme valeur: ', mini)
          #print( 'à l\'emplacement', k)
          basis = min (0,  mini - 0.2 )
     ...
     if len(array_pics)>4:
          ...
          for i in range(0, len(diff_data)):
               sum = sum + y_data[int(diff_data[i])]
          basis= sum/len(diff_data)

          array_pics= arr_short

     #print("La valeur basse retenue est", mean)
     print('fin de basisLow')
     return basis, array_pics