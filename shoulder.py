...
def extrema (max_locaux, y_data):
     ...
     for i in range (1, len(max_locaux)):
          ...
          #print('min_y_value de départ: ', min_y_value)
          ...
          #min_idx = borne_inf + np.argmin(y_data[borne_inf:borne_sup+1])

          #min_idx = borne_inf + np.argmin(y_data[int(borne_inf):int(borne_sup)+1])
          # if the minimum is not one of the extrema (should not happen)
     ...
     if  (max_locaux[len(max_locaux)-1] != len(y_data)-1  and max_locaux[len(max_locaux)-1] != len(y_data)-2)  : 
          #min_idx= max_locaux[len(max_locaux)-1] + np.argmin(y_data[len(max_locaux)-1:len(y_data)-1])
          ...
          array_extrema= np.append(array_extrema, min_idx)
          """if (min_idx != len(max_locaux)-1 and min_idx!= len(y_data)-2): 
               print('min_idx de fin: ')
               print(min_idx)
               array_extrema= np.append(array_extrema, min_idx)"""
     #array_extrema= np.append(array_extrema, len(y_data)-1)
...
def shoulder_list (x_data, y_data, pics, mean ):
     ...
     for e in range (0 , len(extrem)-1):
          ...
          if (extrem[e+1]- extrem[e]> 2*(nb+1)):
               ...
               #for i in range(int(extrem[e]), int(extrem[e+1])):
                    if (i>nb and i<len(y)-nb-1 and y[i] > mean):
                         ...
                         if (i < (nbl+1 ) or i > (len(y)- nbl-1-1) ):
                              ...
                              # steady_change = (yl[i]-yl[i-1])/(yl[i]-yl[i-nbl*2-1])>.95 and (yl[i]-yl[i-1])/(yl[i]-yl[i-nbl*2-1])<1.05 and (yl[i+1]-y[i])/ ( yl[i+2*nbl+ 1]-yl[i])<1.05 and abs(yl[i]-yl[i-1])*1.4 < abs(yl[i+1]-yl[i])
                              # steady_change= (yl[i]-yl[i-1])* nbl > 0.95* ((yl[i]-yl[i-nbl-1])) and (yl[i]-yl[i-1])* nbl < 1.05* ((yl[i]-yl[i-nbl-1])) and abs(yl[i]-yl[i-1])*1.4 < abs(yl[i+1]-yl[i])
                         ...
                         # if ( massive_change) and decreasing_curve :
                         if ( massive_change or steady_change) and decreasing_curve :
                         # if ( (abs(y[i]-y[i-1])*1.4 < abs(y[i+1]-y[i])) and (abs(y[i+1]-y[i])*1.4 < abs(y[i+2]-y[i+1])) and (y[i]-y[i-1])<0 ):
                    ...
                    # during a rise
                         ...
                         if (i < (nbl*2+1 ) or i > (len(y)- nbl*2-1-1) ):
                              ...
                              # steady_change = (yl[i]-yl[i-1])/(yl[i]-yl[i-nbl*2-1])<1.05 and (yl[i+1]-yl[i])/ ( yl[i+2*nb+ 1]-yl[i])<1.05 and abs(yl[i]-yl[i-1])*1.4 < abs(yl[i+1]-yl[i])
                              # steady_change= (yl[i]-yl[i-1])* nbl*2 < 0.95* ((yl[i]-yl[i-nbl*2-1])) and (yl[i]-yl[i-1])* nbl*2 > 1.05* ((yl[i]-yl[i-nbl*2-1])) and abs(yl[i]-yl[i-1])*1.4 > abs(yl[i+1]-yl[i])
                              # steady_change= abs((yl[i]-yl[i-1]) )> 0.9/2* abs((yl[i-1]-yl[i-2-1])) and abs( (yl[i]-yl[i-1]) ) < 1.1/2* abs((yl[i-2-1]-yl[i-1])) and abs(yl[i]-yl[i-1])*1.4 > abs(yl[i+1]-yl[i])
                              steady_change= abs((yl[i]-yl[i-1]) )> 0.9* abs((yl[i-1]-yl[i-2])) and abs( (yl[i]-yl[i-1]) ) < 1.1* abs((yl[i-2]-yl[i-1])) and (abs(yl[i]-yl[i-1])*1.35 > abs(yl[i+1]-yl[i]) or abs(yl[i]-yl[i-1])*1.35 > abs(yl[i+2]-yl[i+1]))
                         if (steady_change): print('steady change ascend')
                         if ( massive_change or steady_change) and ascending_curve :
                         # if ( abs(y[i+1]-y[i])*1.4 < abs(y[i]-y[i-1]) and (abs(y[i- 1]-y[i])*1.43 < abs(y[i-2]-y[i-1])) and (y[i]-y[i-1])>0 ):
                              array_shoulder = np.append(array_shoulder, i)
                              #array_shoulder = np.append(array_shoulder, len(y)-(i+1))
